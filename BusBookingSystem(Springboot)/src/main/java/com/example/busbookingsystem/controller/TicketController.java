...
public class TicketController {
	...
	@GetMapping("/total/{busNumber}")
    public ResponseEntity<Integer> getTotalTicketsBookedForBus(@PathVariable int busNumber) {
        int totalTicketsBooked = ticketService.getTotalTicketsBookedForBus(busNumber);
        return new ResponseEntity<>(totalTicketsBooked, HttpStatus.OK);
    }

	@GetMapping("/bus/{busNumber}")
    public ResponseEntity<List<Ticket>> getTicketsByBusNumber(@PathVariable int busNumber) {
        List<Ticket> tickets = ticketService.findTicketsByBusNumber(busNumber);
